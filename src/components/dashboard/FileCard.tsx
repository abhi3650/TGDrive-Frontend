...
interface FileCardProps {
  ...
  onMenu: (item: FileItem, e: React.MouseEvent) => void; // <--- NEW
}
...
export default function FileCard({ item, onClick, onMenu }: FileCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group glass-card rounded-2xl p-4 relative overflow-visible flex flex-col justify-between aspect-[1/1.1]"
    ...
    >
        ...
        {/* Menu Button (Z-Index High to sit above click handler) */}
        <button 
          onClick={(e) => { e.stopPropagation(); onMenu(item, e); }}
          className="absolute top-3 right-3 p-1.5 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors z-20"
        >
